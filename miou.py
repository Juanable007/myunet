...
def compute_mIoU(gt_dir, pred_dir, png_name_list, num_classes, name_classes):  
    ...
    for ind in range(len(gt_imgs)): 
        ...
        pred = np.where(pred == 255, 0, 1)
        # 读取一张对应的标签，转化成numpy数组
        label = np.array(Image.open(gt_imgs[ind]))
        label = np.where(label == 255, 0, 1)
...
if __name__ == "__main__":
    gt_dir = "./newdata/binaryMasks"
    pred_dir = "./miou_pr_dir"
    png_name_list = open(r"./newdata/val.txt",'r').read().splitlines()
    
    num_classes = 2
    name_classes = ["background","stone"]
